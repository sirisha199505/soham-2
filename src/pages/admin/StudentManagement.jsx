  ...
  if (data?.status === 'completed') {
    ...
    return (
      <div className="flex items-center gap-1">
        ...
        {pct > 0 && <span className="text-[9px] font-semibold px-1 py-0.5 rounded bg-yellow-100 text-yellow-700">best</span>}
      </div>
    );
  }
  if (isOverride) {
                    ...
                    {d?.status === 'completed' ? (
                      <div>
                        <div className="flex items-center justify-end gap-1 mb-0.5">
                          {(d.score?.pct ?? 0) > 0 && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-yellow-100 text-yellow-700">Best</span>}
